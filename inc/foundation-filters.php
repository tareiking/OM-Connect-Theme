...
function om_connect_rename_sticky_post_class( $classes ){
...
add_filter( 'post_class', 'om_connect_rename_sticky_post_class' );

// Add "has-dropdown" CSS class to navigation menu items that have children in a submenu.
function om_connect_nav_menu_item_parent_classing( $classes ) {
...
add_filter( 'nav_menu_css_class', 'om_connect_nav_menu_item_parent_classing', 10, 2 );

// Deletes empty classes and changes the sub menu class name
function om_connect_change_submenu_class( $menu ) {
	$menu = preg_replace( '/ class="sub-menu"/', ' class="dropdown"', $menu );

	return $menu;
}

add_filter( 'wp_nav_menu', 'om_connect_change_submenu_class' );
...
function om_connect_required_active_nav_class( $classes, $item ) {
...
add_filter( 'nav_menu_css_class', 'om_connect_required_active_nav_class', 10, 2 );
...
function om_connect_page_menu() { ?>
	<ul>
	<?php
		wp_list_pages( array(
		    'walker'   => new Om_Connect_Page_Walker,
			'title_li' => ''
		));
	?>
	</ul>
...
class Om_Connect_Page_Walker extends Walker_Page {
	...
	function start_el( &$output, $page, $depth = 0, $args = array(), $current_page = 0 ) {
		...
		if ( '' === $page->post_title )
			$page->post_title = sprintf( __( '#%d (no title)', 'om-connect' ), $page->ID );
...
function om_connect_dequeue_gravity_forms_css() {
...
if ( class_exists( 'GFForms' ) ) {
	add_action( 'init', 'om_connect_dequeue_gravity_forms_css' );
	add_filter( 'gform_validation_message', 'om_connect_gform_form_validation_message', 10, 2 );
	add_filter( 'gform_field_choices', 'om_connect_gform_field_choices', 10, 2);
}
/**
 ...
 * @return mixed
 */
function om_connect_gform_form_validation_message( $validation_message, $form ) {
...
add_filter( 'gform_field_content', 'om_connect_gform_field_content', 10, 5 );


function om_connect_gform_field_content( $content, $field, $value, $lead_id, $form_id ) {
	...
	if ( ! is_admin() ) {
		...
		if ( ( $field['type'] === 'text' ) || ( $field['type'] === 'address' ) || ( $field['type'] === 'name' ) || ( $field['type'] === 'website' )  || ( $field['type'] === 'email' ) || ( $field['type'] === 'textarea' ) || $field['type'] === 'select' ) {
			...
			if ( $field['type'] === 'address' ) {
				...
				$content = str_replace( "{FIELD}", om_connect_gform_get_address_field( $field, $value, 0, $form_id ), $field_content );

			}
			elseif ( $field['type'] === 'name' ) {

				$content = str_replace( "{FIELD}", om_connect_gform_get_name_field( $field, $value, 0, $form_id ), $field_content );

			}
			elseif ( $field['type'] === 'website' ) {

				$content = str_replace( "{FIELD}", om_connect_gform_get_website_field( $field, $value, 0, $form_id ), $field_content );
...
function om_connect_gform_get_website_field( $field, $value, $lead_id, $form_id ) {
...
function om_connect_gform_get_name_field( $field, $value, $lead_id, $form_id ) {
...
function om_connect_name_label ( $classes, $field, $form, $input ) {
...
add_filter( 'om_connect_gforms_name_class', 'om_connect_name_label', 10, 4 );

function om_connect_gform_get_address_field( $field, $value, $lead_id, $form_id ) {
	...
	if ( $address_display_format == "zip_before_city" ) {
		...
		$style = ( IS_ADMIN && rgget( "hideState", $field ) ) ? "style='display:none;'" : "";
		if ( IS_ADMIN || ! rgget( "hideState", $field ) ) {
			$state_field = om_connect_gform_get_state_field( $field, $id, $field_id, $state_value, $disabled_text, $form_id, $state_label );
	...
	$city = "<div class='" . apply_filters( 'om_connect_gforms_address_city_class', 'large-6 columns', $field, $form_id ) . "'>{$city}</div>";

	//Wrap state in foundation divs
	$state = "<div class='" . apply_filters( 'om_connect_gforms_address_state_class', 'large-6 columns', $field, $form_id ) . "'>{$state}</div>";

	//Wrap ZIP in foundation divs
	$zip = "<div class='" . apply_filters( 'om_connect_gforms_address_zip_class', 'large-6 columns', $field, $form_id ) . "'>{$zip}</div>";
...
function om_connect_gform_get_state_field( $field, $id, $field_id, $state_value, $disabled_text, $form_id, $state_label ) {
...
function om_connect_gform_field_choices( $choices, $field ) {
...
add_action( 'gform_field_css_class', 'om_connect_foundation_custom_class', 10, 3);

function om_connect_foundation_custom_class( $classes, $field, $form ){
...
function om_connect_change_first_name( $label, $form_id ){
    return "First Name";
}
add_filter( 'gform_name_first', 'om_connect_change_first_name', 10, 2 );

function om_connect_change_last_name( $label, $form_id ){
    return "Last Name";
}
add_filter( 'gform_name_last', 'om_connect_change_last_name', 10, 2 );

add_filter( 'gform_confirmation_anchor', create_function( '', 'return true;' ) );

function om_connect_flex_video( $html, $url, $attr ) {
...
add_filter( 'embed_oembed_html', 'om_connect_flex_video', 99, 3 );
 ...
 * @return mixed
 */
function om_connect_cat_count_span( $links ) {
  $links = str_replace( '</a> (', ' (', $links );
  $links = str_replace( ')', ')</a>', $links );
  return $links;
}
add_filter( 'wp_list_categories', 'om_connect_cat_count_span' );
 ...
 * @return mixed
 */
function om_connect_archive_count_span( $links ) {
  $links = str_replace( '</a>&nbsp;(', ' (', $links );
  $links = str_replace( ')', ')</a>', $links );
  return $links;
}
add_filter( 'get_archives_link', 'om_connect_archive_count_span' );
 ...
 * @return mixed
 */
function om_connect_pagination( $wp_pagenavi_output ) {
	...
	$wp_pagenavi_output = apply_filters( 'om_connect_pagination', $wp_pagenavi_output );

	return $wp_pagenavi_output;
}
add_filter( 'wp_pagenavi', 'om_connect_pagination' );